...
export default function PublicRegistration({ event }: { event: EventItem }) {
  ...
  return <><header className="event-header"><div className="event-header-content">{event.logo ? <img className="event-logo" src={event.logo} alt={event.name} /> : <strong className="event-brand">{event.name}</strong>}<label style={{ marginLeft: "auto" }}>{t.language}<select value={locale} onChange={(e) => setLocale(e.target.value as PublicLocale)}><option value="pt-BR">PT-BR</option>{publicLocales.filter((item) => item !== "pt-BR").map((item) => <option key={item} value={item}>{item.toUpperCase()}</option>)}</select></label></div></header>{event.banner && <section className="event-banner"><img src={event.banner} alt={event.name} /></section>}<section className={`confirmation-section ${event.banner ? "has-banner" : "without-banner"}`}><header className="confirmation-heading"><small>{t.registration.toUpperCase()}</small><h1>{event.name}</h1><p>{event.description || t.introduction}</p>{event.location && <p><strong>{event.location}</strong>{event.startInfo ? ` · ${event.startInfo}` : ""}</p>}</header><div className="event-card">{done ? <section className="event-message"><h2>{t.successTitle}</h2><p>{t.successText}</p><p>{t.emailNotice}</p></section> : <form onSubmit={submit}><h2>{t.register}</h2><p>{t.introduction}</p><div className="identity"><label>{t.name} *<input value={name} autoComplete="name" onChange={(e) => setName(e.target.value)} /></label><label>{t.email} *<input type="email" value={email} autoComplete="email" onChange={(e) => setEmail(e.target.value)} /></label><label>{t.company} <small>({t.optional})</small><input value={company} autoComplete="organization" onChange={(e) => setCompany(e.target.value)} /></label><label>{t.phone} <small>({t.optional})</small><input value={phone} autoComplete="tel" onChange={(e) => setPhone(e.target.value)} /></label></div><div className="dynamic-form-fields">{fields.map(renderField)}</div>{error && <div className="error" role="alert">{error}</div>}<button className="submit" disabled={loading} type="submit">{loading ? t.submitting : t.submit}</button></form>}</div></section></>;
}
